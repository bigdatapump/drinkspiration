...
$(document).ready(function () {
    ...
    if ($("body").hasClass("slider-bg")) {
        $(".hero").vegas({
            ...
            color: "#fffde6",
            slides: [
				{ src: "img/slider/Drinkspiration_bg_1.jpg" },
				{ src: "img/slider/Drinkspiration_bg_2.jpg" },
				{ src: "img/slider/Drinkspiration_bg_3.jpg" }				
			]
        });
...
const createAvatar = function(name){
    $('#user').text(name);
};

$(function(){
    $.getJSON(document.location.origin+'/.auth/me', (data) => {
    let username = data[0].user_id.split('@')[0].split('.').map((x) => x.charAt(0).toUpperCase() + x.slice(1)).join(' ');
    createAvatar(username, data[0].user_id);
    }).error((err) => createAvatar('User Name'));
});